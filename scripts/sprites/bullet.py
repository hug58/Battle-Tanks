...
class Bullet(pg.sprite.Sprite,Animation):
    ...
    def __init__(self,position,angle,num_player):
        ...
        self._delay = 2
        self.image.fill((2,218,136)) if self._num_player == 0 else self.image.fill((188,2,218))
    ...
    @property
    def done(self):
        """ return True if bullet is done, well, the bullet has already explotation"""
        return super()._done
    
    @property
    def num_player(self):
        """ this is  index of player, it will use for bullet can distinguish between player and other players"""
